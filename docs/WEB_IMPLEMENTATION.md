...
| `Player.configurePlayer(options)` | ⚠️ | Stub normalized options (default `playerType: 'application'`, `mixWithOthers: false`); **no** AVAudioSession / SystemMusicPlayer — see [PLAYBACK.md](./PLAYBACK.md) |
...
| `configurePlayer` | No-op audio session; return stub normalized options (`playerType` default `application`, `mixWithOthers: false`) |
